...
class MainWindow(QMainWindow):
    def __init__(self, project_root: Path) -> None:
        ...
        self.setWindowTitle("Dong 微电网能源管理系统 v1.6")
    ...
    def _refresh_window_title(self) -> None:
        ...
        self.setWindowTitle(f"{self.project_name}{marker} - Dong 微电网能源管理系统 v1.6")
        if hasattr(self, "project_meta"):
            state = "未保存" if self.modified else "已保存"
            if self.project_path is None and not self.modified:
                state = "新项目"
                self.project_meta.setText(f"{self.project_name}{marker}  |  {state}  |  v1.6")
