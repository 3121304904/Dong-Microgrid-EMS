...
>"%LOG_FILE%" echo [Dong v1.6] Startup started at %date% %time%
...
if exist "%APP_ROOT%dist\DongMicrogridEMS\DongMicrogridEMS.exe" (
    echo Starting the portable Dong v1.6 application...
    >>"%LOG_FILE%" echo Launch mode: portable EXE
    "%APP_ROOT%dist\DongMicrogridEMS\DongMicrogridEMS.exe"
    set "EXIT_CODE=%ERRORLEVEL%"
    if not "%EXIT_CODE%"=="0" (
        echo.
        echo Dong v1.6 failed to start. Exit code: %EXIT_CODE%
...
where py >nul 2>nul
if %ERRORLEVEL%==0 (
    echo Starting Dong v1.6 with Python launcher...
...
if %ERRORLEVEL%==0 (
    echo Starting Dong v1.6 with system Python...
