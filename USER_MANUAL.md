...
Dong v1.6 用于模拟一个包含光伏、储能、柴油发电机、园区负荷和公共电网的微电网。软件可以读取 96 个 15 分钟时段的数据，生成确定性、风险感知和滚动预测调度计划，在模拟实际光伏上执行实时功率平衡，并通过回放、滚动调度中心、统计实验和运行报告解释结果。v1.6 还支持 50Hertz 德国区域光伏年度数据，并改进了双击启动脚本的兼容性和故障诊断。
...
v1.6 的 `run.bat` 会优先启动便携版 EXE，不要求系统安装 Python；找不到 EXE 时才会依次尝试 `.venv`、`py -3` 和 `python`。如果启动失败，CMD 会停留并显示错误，详细信息保存在 `output\startup.log`。仍出现问题时，先把整个发布目录解压到 `C:\Dong_v1.6` 验证；不要从 ZIP 预览窗口内部运行。

### 路径包含中文时启动失败

便携版通常可以直接从中文路径运行。若某台电脑仍有路径兼容性问题，先将完整发布目录解压到 `C:\Dong_v1.6` 再测试；不要使用过长路径，也不要只复制 `DongMicrogridEMS.exe` 而遗漏 `_internal`。
