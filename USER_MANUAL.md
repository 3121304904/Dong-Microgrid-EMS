...
Dong v1.7 用于模拟一个包含光伏、储能、柴油发电机、园区负荷和公共电网的微电网。软件可以读取 96 个 15 分钟时段的数据，生成确定性、风险感知和滚动预测调度计划，在模拟实际光伏上执行实时功率平衡，并通过回放、滚动调度中心、统计实验和运行报告解释结果。v1.7 还支持 50Hertz 德国区域光伏年度数据，并改进了双击启动脚本的兼容性和故障诊断。
...
首次使用双击 `setup.bat`，等待出现安装完成提示；以后双击 `run.bat` 即可。脚本会优先打开 `dist\DongMicrogridEMS\DongMicrogridEMS.exe`，因此发布文件夹不要求本机安装 Python；如果找不到 EXE，才会回退到 `.venv` 或系统 Python。
...
![滚动调度中心](docs/screenshots/rolling.png)

该页用五步流程图说明滚动策略：读取刚发生的光伏误差，更新 EWMA 偏差，修正未来 16 个点（4 小时）的预测，用动态规划重新安排储能、主网和柴油机功率，最后只执行当前 4 个点（1 小时）。下一小时获得新实测值后再次重复。图中的竖线表示重优化时刻，主网负载率达到 82% 时标记为“电网紧张”，高价时段进一步标记为“高价削峰”，供电不足时标记为“保供告警”。
...
v1.7 的 `run.bat` 会优先启动便携版 EXE，不要求系统安装 Python；找不到 EXE 时才会依次尝试 `.venv`、`py -3` 和 `python`。如果启动失败，CMD 会停留并显示错误，详细信息保存在 `output\startup.log`。仍出现问题时，先把整个发布目录解压到 `C:\Dong_v1.7` 验证；不要从 ZIP 预览窗口内部运行。

### 路径包含中文时启动失败

便携版通常可以直接从中文路径运行。若某台电脑仍有路径兼容性问题，先将完整发布目录解压到 `C:\Dong_v1.7` 再测试；不要使用过长路径，也不要只复制 `DongMicrogridEMS.exe` 而遗漏 `_internal`。
...
进入“滚动调度中心”后，可以用“播放”自动推进 96 个 15 分钟时段，也可以用“单步”逐点演示。拖动“时间游标”会立即刷新当前控制模式、主网状态、EWMA 偏差、误差 σ、当前预测窗和储能动作。图中的橙色竖线表示当前时刻，浅黄色区域表示未来 4 小时预测窗，紫色曲线表示上一时段光伏预测误差。

建议讲解顺序：先点击“复位”，再以 4x 播放；在出现重规划标线时暂停，说明程序只使用已经发生的误差更新 EWMA，然后调用滚动动态规划，最后只发布当前 1 小时计划。公式、状态转移矩阵和 Mermaid 流程图见 docs/rolling_logic.md。
