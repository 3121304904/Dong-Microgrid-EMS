...
Dong v1.3 用于模拟一个包含光伏、储能、柴油发电机、园区负荷和公共电网的微电网。软件可以读取 96 个 15 分钟时段的数据，生成确定性和风险感知日前计划，在模拟实际光伏上执行实时功率平衡，并通过回放、统计实验和运行报告解释结果。v1.3 还支持 50Hertz 德国区域光伏年度数据，并修复页面切换残影。
...
柱形图比较确定性与风险感知策略的实际成本、主网购电量和碳排放；表格给出成本、购电、柴油、储能吞吐、弃光和失负荷的差值。两套策略在同一条模拟实际光伏曲线上执行，因此比较是公平的。
...
表格逐行列出时间、负荷、光伏预测/实测、储能、SOC、柴油机、购电、上网和电价。它适合在答辩时证明程序确实计算了 96 个时段，也可用于核对某一回放时刻。
...
v1.3 已按中文路径测试。仍出现问题时，先把整个发布目录解压到 `C:\Dong_v1.3` 验证；不要使用过长路径，也不要从只读压缩包内部运行。
