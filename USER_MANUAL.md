...
Dong v1.5 用于模拟一个包含光伏、储能、柴油发电机、园区负荷和公共电网的微电网。软件可以读取 96 个 15 分钟时段的数据，生成确定性、风险感知和滚动预测调度计划，在模拟实际光伏上执行实时功率平衡，并通过回放、滚动调度中心、统计实验和运行报告解释结果。v1.5 还支持 50Hertz 德国区域光伏年度数据。
...
v1.5 已按中文路径测试。仍出现问题时，先把整个发布目录解压到 `C:\Dong_v1.5` 验证；不要使用过长路径，也不要从只读压缩包内部运行。
