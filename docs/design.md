...
系统围绕“输入场景 → 构造日前计划 → EWMA 滚动预测修正 → 每小时滚动重调度 → 实时功率平衡 → 统计与实验 → 保存/导出”形成可演示、可测试的完整闭环。界面仅负责交互与展示，调度、实验、项目和报告均为独立 Python 模块，可在不启动 Qt 的情况下测试。v1.7 额外保证便携 EXE 优先启动，并在批处理入口保留启动日志和错误信息。
...
RollingPage 在同一个页面内维护 96 点时间游标和 QTimer 回放。每次游标移动，图表更新当前线、预测窗阴影、误差带和状态卡片；调度器通过 DataFrame 字段及 DispatchResult.diagnostics 暴露误差、波动尺度与重规划轨迹。

详细公式见 docs/rolling_logic.md。
