...
系统围绕“输入场景 → 构造日前计划 → EWMA 滚动预测修正 → 每小时滚动重调度 → 实时功率平衡 → 统计与实验 → 保存/导出”形成可演示、可测试的完整闭环。界面仅负责交互与展示，调度、实验、项目和报告均为独立 Python 模块，可在不启动 Qt 的情况下测试。v1.6 额外保证便携 EXE 优先启动，并在批处理入口保留启动日志和错误信息。
